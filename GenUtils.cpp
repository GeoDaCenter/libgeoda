...
bool GdaAlgs::RateSmoother_SEBS(int obs,
                                GeoDaWeight* w,
                                double *P, double *E,
                                double *results,
                                std::vector<bool>& undefined)
{
    ...
    for (int i=0; i<obs; i++) {
        ...
        int  nbrs = w->GetNbrSize(i);
        const std::vector<long>& elt_i = w->GetNeighbors(i);
        ...
        if (nbrs > 0) {
            ...
            if (!undefined[i]) {
                ...
                if (theta2 < 0) theta2 = 0.0;
                q1 = (theta2 + (theta1/P[i]));
                weight = (q1 > 0) ? theta2 / q1 : 1;
                results[i] = (weight * pi_raw[i]) + ((1-weight) * theta1);
            }
        } else {
            undefined[i] = true;
            results[i] = 0;
...
bool GdaAlgs::RateSmoother_SRS(int obs,
                               GeoDaWeight* w,
                               double *P, double *E,
                               double *results, std::vector<bool>& undefined)
{
    ...
    for (int i=0; i<obs; i++) {
        ...
        int sz = w->GetNbrSize(i);
        const std::vector<long>& elm_i = w->GetNeighbors(i);
        for (int j=0; j<sz; j++) {
            SE += E[elm_i[j]];
            SP += P[elm_i[j]];
        }
